...
app.use(session);
app.get("/status", (req, res) => res.status(200).json({ message: "healthy" }));
    ...
    try {
        ...
        const errorHandler = (err, req, res, next) => {
            ...
            res.status(500).send(`<h1>Internal Server Error</h1>
				...
				<b>${trackingId}</b></p>`);
            logger.error('Express error handler dump with tracking ID: %s, error dump: %o', trackingId, err);
        };
        // eslint-disable-next-line no-unused-vars
...
function setupSAML() {
    samlStrategy = new SAMLStrategy(config.auth.saml, function (profile, done) {
        return done(null, {
            id: profile.uid,
            _userinfo: profile
        });
...
function setupLocal() {
    localStrategy = new LocalStrategy(function (username, plaintextPassword, done) {
        ...
        if (found === undefined)
            ...
            const userinfo = Object.assign({}, found);
...
function setupOIDC(oidcIssuer) {
    ...
    const usePKCE = false;
    oidcStrategy = new Strategy({ client: oidcClient, params, passReqToCallback, usePKCE }, (tokenset, userinfo, done) => {
...
async function setupAuth() {
    // LTI
    if (typeof (config.auth.lti) !== 'undefined' &&
    ...
    // lti launch
    app.post('/auth/lti', passport.authenticate('lti', { failureRedirect: '/' }), (req, res) => {
    ...
    app.get('/auth/metadata', (req, res) => {
        ...
        if (config.auth && config.auth.saml &&
            ...
            const metadata = samlStrategy.generateServiceProviderMetadata(config.auth.saml.decryptionCert, config.auth.saml.signingCert);
            if (metadata) {
    ...
    // callback
    app.all('/auth/callback', passport.authenticate(authStrategy, { failureRedirect: '/auth/login' }), async (req, res, next) => {
...
async function runWebSocketServer() {
    ...
    io.on('connection', async (socket) => {
        ...
        const validRoom = await roomController.validateRoom(roomId);
        const validMeeting = validRoom.length > 0 && await roomController.validateMeeting(roomId);
        if (validRoom.length <= 0) {
            io.to(socket.id).emit("meeting_checks", { message: "Invalid_meeting" });
            socket.disconnect(true);
            return;
        }
        else if (validMeeting == 'before_meeting') {
            io.to(socket.id).emit("meeting_checks", { message: "before_meeting" });
            socket.disconnect(true);
            return;
        }
        else if (validMeeting == "after_meeting") {
            io.to(socket.id).emit("meeting_checks", { message: "after_meeting" });
            socket.disconnect(true);
            return;
        }
        validMeeting.length > 0 && io.to(socket.id).emit("valid_meeting", { room: validRoom[0] });
        ...
        queue.push(async () => {
            ...
            if (Boolean(socket.handshake.session.passport) &&
                Boolean(socket.handshake.session.passport.user)) {
                const { id, displayName, picture, email, _userinfo } = socket.handshake.session.passport.user;
                peer.authId = id;
