...
const logger = new Logger_1.default('Lobby');
class Lobby extends EventEmitter {
    ...
    parkPeer(peer) {
        ...
        peer.socketRequestHandler = (request, cb) => {
            logger.debug('Peer "request" event [method:"%s", peer:"%s"]', request.method, peer.id);
            if (this._closed)
                return;
    ...
    async _handleSocketRequest(peer, request, cb) {
        logger.debug('_handleSocketRequest [peer:"%s"], [request:"%s"]', peer.id, request.method);
        if (this._closed)
            return;
