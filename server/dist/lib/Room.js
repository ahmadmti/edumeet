...
const roomController = require("../lib/roomCheck");
const access_1 = require("./access/access");
const permissions = require('./access/perms'), { CHANGE_ROOM_LOCK, PROMOTE_PEER, MODIFY_ROLE, SEND_CHAT, MODERATE_CHAT, SHARE_AUDIO, SHARE_VIDEO, SHARE_SCREEN, EXTRA_VIDEO, SHARE_FILE, MODERATE_FILES, MODERATE_ROOM, LOCAL_RECORD_ROOM } = permissions;
const { config } = require('./config/config');
const logger = new Logger_1.default('Room');
// In case they are not configured properly
const roomAccess = Object.assign({ [access_1.BYPASS_ROOM_LOCK]: [userRoles.ADMIN], [access_1.BYPASS_LOBBY]: [userRoles.NORMAL] }, config.accessFromRoles);
const roomPermissions = Object.assign({ [CHANGE_ROOM_LOCK]: [userRoles.NORMAL], [PROMOTE_PEER]: [userRoles.NORMAL], [MODIFY_ROLE]: [userRoles.MODERATOR], [SEND_CHAT]: [userRoles.NORMAL], [MODERATE_CHAT]: [userRoles.MODERATOR], [SHARE_AUDIO]: [userRoles.NORMAL], [SHARE_VIDEO]: [userRoles.NORMAL], [SHARE_SCREEN]: [userRoles.NORMAL], [EXTRA_VIDEO]: [userRoles.NORMAL], [SHARE_FILE]: [userRoles.NORMAL], [MODERATE_FILES]: [userRoles.MODERATOR], [MODERATE_ROOM]: [userRoles.MODERATOR], [LOCAL_RECORD_ROOM]: [userRoles.NORMAL] }, config.permissionsFromRoles);
...
class Room extends EventEmitter {
        ...
        for (const worker of allMediasoupWorkersOnServer.values()) {
            for (const routerId of worker.appData.routers.keys()) {
                if (workerLoads.has(worker.pid)) {
                    ...
                    workerLoads.set(worker.pid, (routerLoads.has(routerId) ? routerLoads.get(routerId) : 0));
                }
            }
        }
    ...
    constructor({ roomId, mediasoupRouters, audioLevelObservers, mediasoupWorkers, peers }) {
    ...
    handlePeer({ peer, returning }) {
        ...
        if (this._peers[peer.id]) {
            logger.warn('handleConnection() | there is already a peer with same peerId [peer:"%s"]', peer.id);
        }
        // Returning user
        if (returning)
            this._peerJoining(peer, true);
        // Has a role that is allowed to bypass room lock
        else if (this._hasAccess(peer, access_1.BYPASS_ROOM_LOCK))
            this._peerJoining(peer);
        else if (config.maxUsersPerRoom &&
            (Object.keys(this._peers).length +
                this._lobby.peerList().length) >= config.maxUsersPerRoom) {
            ...
            this._hasAccess(peer, access_1.BYPASS_LOBBY) ?
                this._peerJoining(peer) :
    ...
    _handleLobby() {
        ...
        this._lobby.on('peerRolesChanged', (peer) => {
            // Has a role that is allowed to bypass room lock
            if (this._hasAccess(peer, access_1.BYPASS_ROOM_LOCK)) {
    ...
    logStatus() {
        logger.info('logStatus() [room id:"%s", peers:"%s"]', this._roomId, Object.keys(this._peers).length);
    }
    dump() {
        return {
            roomId: this._roomId,
    ...
    selfDestructCountdown() {
        ...
        this._selfDestructTimeout = setTimeout(() => {
            ...
            if (this.checkEmpty() && this._lobby.checkEmpty()) {
                ...
                logger.info('Room deserted for some time, closing the room [roomId:"%s"] and kick peers from the lobby', this._roomId);
                this.close();
            }
            else
        ...
        this._queue.push(async () => {
            ...
            if (returning) {
                ...
                if (config.turnAPIURI) {
                    try {
                        const { data } = await axios.get(config.turnAPIURI, {
                            ...
                            params: Object.assign(Object.assign({}, config.turnAPIparams), { 'api_key': config.turnAPIKey, 'ip': peer.socket.request.connection.remoteAddress })
                        });
                        turnServers = [{
                                urls: data.uris,
                ...
                if (config.activateOnHostJoin && this._lobby.peerList().length > 0 &&
                    !this._locked && peer.roles.some((role) => config.permissionsFromRoles.PROMOTE_PEER.includes(role))) {
    ...
    _handlePeer(peer) {
        ...
        peer.socket.on('request', (request, cb) => {
            logger.debug('Peer "request" event [method:"%s", peerId:"%s"]', request.method, peer.id);
    ...
    _handlePeerClose(peer) {
        ...
        const hasPromotePeer = peer.roles.some((role) => roomPermissions[PROMOTE_PEER].some((roomRole) => role.id === roomRole.id));
            ...
            case 'join':
                {
                    ...
                    const { displayName, picture, rtpCapabilities, returning } = request.data;
                    ...
                    for (const otherPeer of this.getJoinedPeers(peer)) {
                        this._notification(otherPeer.socket, 'newPeer', Object.assign(Object.assign({}, peer.peerInfo), { returning }));
                    }
                    logger.debug('peer joined [peer: "%s", displayName: "%s", picture: "%s"]', peer.id, displayName, picture);
                    break;
                }
            case 'createWebRtcTransport':
                {
                    ...
                    const webRtcTransportOptions = Object.assign(Object.assign({}, config.mediasoup.webRtcTransport), { appData: { producing, consuming } });
            ...
            case 'produce':
                {
                    ...
                    try {
                        ...
                        throw new Error('transport.produce failed: [kind: "%s", rtpParameters: "%o", appData: "%o", error: "%o"]', kind, rtpParameters, appData, error);
                    }
                    ...
                    if (kind === 'audio') {
                        this._audioLevelObservers.get(peer.routerId).audioLevelObserver.addProducer({ producerId: producer.id }).catch((error) => {
            ...
            case 'closeProducer':
                {
                    ...
                    this._audioLevelObservers.get(peer.routerId).audioLevelObserver.removeProducer({ producerId: producer.id }).catch((error) => {
            ...
            case 'sendFile':
                {
                    ...
                    this._fileHistory.push(Object.assign({}, file));
                    // Spread to others
                    this._notification(peer.socket, 'sendFile', Object.assign({}, file), true);
                    // Return no error
                    cb();
                    break;
                }
    ...
    async _createConsumer({ consumerPeer, producerPeer, producer }) {
        logger.debug('_createConsumer() [consumerPeer:"%s", producerPeer:"%s", producer:"%s"]', consumerPeer.id, producerPeer.id, producer.id);
    ...
    _hasPermission(peer, permission) {
        const hasPermission = peer.roles.some((role) => roomPermissions[permission].some((roomRole) => role.id === roomRole.id));
        if (hasPermission)
            return true;
    ...
    _hasAccess(peer, access) {
        return peer.roles.some((role) => roomAccess[access].some((roomRole) => role.id === roomRole.id));
    }
    /**
    ...
    _getPeersWithPermission(permission = null, excludePeer = undefined, joined = true) {
        return Object.values(this._peers)
            .filter((peer) => peer.joined === joined &&
            peer !== excludePeer &&
            peer.roles.some((role) => roomPermissions[permission].some((roomRole) => role.id === roomRole.id)));
    }
    ...
    _sendRequest(socket, method, data = {}) {
        return new Promise((resolve, reject) => {
            socket.emit('request', { method, data }, this._timeoutCallback((err, response) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async _pipeProducersToRouter(routerId) {
        ...
        const peersToPipe = Object.values(this._peers)
    ...
    async _getRouterId() {
        const routerId = Room.getLeastLoadedRouter(this._mediasoupWorkers, this._allPeers, this._mediasoupRouters);
    ...
    _getRoutersToPipeTo(originRouterId) {
        return Object.values(this._peers)
            ...
            .filter((routerId, index, self) => routerId !== originRouterId && self.indexOf(routerId) === index);
    }
}
module.exports = Room;
