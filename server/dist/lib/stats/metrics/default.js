...
const logger = new Logger_1.default('metrics:default');
    ...
    for (const worker of workers.values()) {
        ...
        for (const router of worker.appData.routers.values()) {
            ...
            for (const [transportId, transport] of router.appData.transports) {
                ...
                const localAddr = await addr(iceSelectedTuple.localIp, iceSelectedTuple.localPort);
                const remoteAddr = await addr(iceSelectedTuple.remoteIp, iceSelectedTuple.remotePort);
                for (const [producerId, producer] of transport.appData.producers) {
                    logger.debug(`visiting producer ${producerId}`);
                    const { roomId, peerId, displayName, userAgent, kind, codec } = commonLabels(producer, (peer) => peer._producers.has(producerId));
                ...
                for (const [consumerId, consumer] of transport.appData.consumers) {
                    ...
                    const { roomId, peerId, displayName, userAgent, kind, codec } = commonLabels(consumer, (peer) => peer._consumers.has(consumerId));
