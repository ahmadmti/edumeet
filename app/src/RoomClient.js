...
export default class RoomClient
{
	...
	*/
	...
	async join({ roomId, joinVideo, joinAudio })
	{
		...
		this._signalingSocket.on('meeting_checks', (data) =>
		{
			...
			// alert('stop');
			// store.dispatch(notificationActions.addNotification({ message: data.message }));
		});
		this._signalingSocket.on('valid_meeting', (data) =>
		{
			// eslint-disable-next-line no-alert

			store.dispatch(roomActions.isValidMeeting(data.room));
		});
